...
To realize this, consider constructing a multipodal graphon with $V = abs(V(G))$ podes, where each pode corresponds to a vertex of $G$. We scale the sizes of these podes and the weights of the edges between them as powers of $tau$. Let the measure of the $v$-th pode be $tau^(x_(v))$ for some $x_(v) >= 0$, let the edge weight between pode $u$ and $v$ be $1$  (for non edges in $G$, set the edge weight to 0).
...
$
  t(G, W) approx product_(v in V(G)) tau^(x_(v)) = tau^(sum_(v in V(G))^() x_(v))
$

#figure(
  ansatz-construction-canvas,
  caption: [The structural mapping of a bowtie graph into the candidate multipodal graphon. The interval widths represent the vertex constraints ($x_i$), and the block densities represent the edge constraints ($y_(i j)$).],
)

Because $tau$ is small ($tau -> 0$), to _maximize_ this density, we must minimize the exponent $sum_(v in V(G))^() x_(v)$. However, we have a restricted total triangle budget. In this $V$-podal graphon, the triangles formed correspond exactly to $T in cal(T)(G)$. The density of any specific triangle $T$ in the graphon scales as:
$ tau^(sum_(v in V(T))^() x_(v)^(*)). $
...
#definition[
  Let $alpha: cal(G) -> RR_(>0)$ where $ alpha(G) := min{sum_(v in V(G))^() x_(v)} \ $ where $x: V(G) -> RR_(>=0)$, and for all triangles $T in cal(T)(G)$, $ sum_(v in V(tau))^() x_(v) >= 1. $
]
...
#proof[
  ...
  Fix some $T_(0) in cal(T)(G)$. Let $S = {x_(v)^(*) mid(|) v in V(T_(0))}$. Let $z = max S$ and $v_(0)$ be the vertex which gives $z$.

  $ (abs(V(G)) + abs(E(G)))z & >= sum_(v in V(T_(0)))^() x_(v)^(*) + sum_(e in E(T_(0)))^() y_(e)^(*) >= 1 + delta. $ Thus $ z >= (1 + delta)/(abs(V(G)) + abs(E(G))) > 0 $

  Let $delta prime = min(z, delta)$. Let $tilde(x)$ be identical to $x^(&)$ except at $v_(0)$ where $tilde(z)_(v_(0)) = z_(v_(0)) - delta prime$. The only sums which change are the sums which $v_(0)$ is a part of. For all $T in cal(T)(G)$ such that $v_(0) in V(T)$,
...
#proof[
  ...
  $ sum_(v in V(G))^() x_(v) $ and for all triangles $T in cal(T)(G),$ $ sum_(v in V(T))^() x_(v) >= 1. $
  By @trispandecomp, no vertex of $L$ lies in triangle, so for every $v in L$, $x_(v)$ appears in no constraint. Hence at the optimum we may take $x_(v) = 0$. Thus $alpha(G)$ equals the optimal value of

  $ sum_(v in V(cal(C)))^() x_(v) $

  subject to the same constraints. By @trispandecomp, because $V(cal(C))$ can be partitioned into ${V(C)}_(C in cal(C))$,

  $
    sum_(v in V(cal(C)))^() x_(v) = sum_(C in cal(C))(sum_(v in V(C))^() x_(v)).
  $

  Moreover, every triangle $T in cal(T)(G)$ lives entirely within a unique $C in cal(C)$. Conversely for all $C in cal(C)$, every triangle in $C$ is a triangle in $G$. So the constraint set decouples: the constraints involving variables from a fixed $C in cal(C)$ are exactly the triangle constraints of $cal(T)(C)$, and no constraint mixes variables from different components. Hence the LP seperates into a sum of $n$ independent sub-LPs, one per component, and $ alpha(G) & = sum_(C in cal(C)) min{sum_(v in V(C))^() x_(v) mid(|) x >= 0, x "feasible"} \
...
#proof[
  ...
  Let $W_(triangle): [0, 1]^(3) -> [0, 1]$ where $ W_(triangle)(x_(1), x_(2), x_(3)) = W(x_(1), x_(2)) W(x_(2), x_(3)) W(x_(1), x_(3)). $

  For all $e = {u, v} in E(G)$, $ sum_(T in.rev e)^() z_(T)^(*) & = sum_(T in.rev u \ T in.rev v)^() z_(T)^(*) <= sum_(T in.rev v)^() z_(T)^(*). $

  By the Dual LP $sum_(T in.rev v)^() z_(T)^(*) in [0, 1]$ for $v in V(G)$, thus $sum_(T in.rev e)^() z_(T)^(*) in [0, 1]$ for all edges $e in E(G)$. Since $W <= 1$, raising $W$ to a exponent in $[0, 1]$ can only increase the value. Thus $ product_(i j in E(G)) W(x_(i), x_(j)) <= product_(i j in E(G)) W(x_(i), x_(j))^(sum_(T in.rev i j) z_(T)^(*)) = product_(T in cal(T)(G)) W_(triangle)(x_(T))^(z_(T)^(*)), $
